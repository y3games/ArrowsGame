...
> 이후 무작위 대기열 매칭은 방 기능으로, 개인 보드·시간 경쟁 규칙은 공유 보드 턴제로 대체되었다
> ([긴 화살표·공유 보드·턴제](2026-09-25-snake-arrows-turn-based.md)). 이 문서의 `applyAttempt`, 10초 시도 창,
> 링 재충전 설명은 당시 기준이다. 무작위 대기열 매칭은 방 기능으로 대체되었다. 아래 "다시 하기"/`queue:join` 설명은 당시 기준이며
